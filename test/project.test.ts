...
// ---------------------------------------------------------------------------
// A projected swap goes through the weapon-eligibility gate
// ---------------------------------------------------------------------------

/**
 * Kraken's five stars need a two-handed weapon, and §7 projects a swap by
 * re-aggregating a mutated save — so a projection that puts a one-hander in the
 * main hand has to lose them, exactly as wearing it would. This is the case the
 * eligibility repair could most easily miss, because nothing in the projection
 * code mentions weapons: it inherits the rule or it does not have it.
 *
 * The spear case is the other half. The two-handed spear field is `Spear2h`,
 * and the accepted-weapon vocabulary read `Spear`, so every spear restriction
 * was invisible. With the gate switched on, that would have taken Kraken off a
 * spear user — a regression the gate itself introduces.
 */
describe('projected swaps and weapon eligibility', () => {
  const TWO_HANDER = 'records/items/gatedsword2h.dbr';
  const ONE_HANDER = 'records/items/gatedsword1h.dbr';
  const SPEAR = 'records/items/gatedspear2h.dbr';
  const STAR = 'records/skills/devotion/gatedstar.dbr';
  ...
  const gateDb = stubDb({
    ...
    skills: {
      [STAR]: skill(STAR, {
        ...
        stats: { characterAttackSpeedModifier: 26, defensiveFire: 30 },
      }),
    },
  });

  /** Two-hander held, the one-hander and the spear loose in bag 1. */
  function gateWorld(): { input: ProjectionInput; save: CharacterSave } {
    const bagOne = { ...instance({ baseName: ONE_HANDER, seed: 21 }), x: 0, y: 0 };
    const bagSpear = { ...instance({ baseName: SPEAR, seed: 22 }), x: 1, y: 0 };
    const theSave = save({
      weaponSet1: [instance({ baseName: TWO_HANDER, seed: 20 }), null],
      inventorySacks: [[bagOne, bagSpear]],
      devotions: [characterSkill(STAR, 1)],
    });
    const itemsById = new Map<string, ResolvedItem>([
      ['cand-1h', resolveItem(bagOne, gateDb, 'inventory', 'bag 1 (0,0)', undefined, { kind: 'inventory', sack: 0, x: 0, y: 0 })],
      ['cand-spear', resolveItem(bagSpear, gateDb, 'inventory', 'bag 1 (1,0)', undefined, { kind: 'inventory', sack: 0, x: 1, y: 0 })],
    ]);
    return {
      save: theSave,
      input: { save: theSave, account: {}, db: gateDb, difficulty: 'Ultimate', itemsById, socketablesById: new Map() },
    };
  }

  const equipInMainHand = (targetId: string): PlanVerdict[] => [
    { slot: 'Main hand', itemId: '', verdict: 'EQUIP', targetId, reason: '' },
  ];

  it('loses a two-hander-gated star when the projection equips a one-hander', () => {
    const { input, save: source } = gateWorld();
    const result = projectVerdicts(equipInMainHand('cand-1h'), input)!;

    expect(result.projection.skipped).toEqual([]);
    expect(result.before.speed.attack.permanentPercent).toBe(26);
    expect(result.after.speed.attack.permanentPercent).toBe(0);
    expect(result.before.resistances.effective.fire).toBe(30);
    expect(result.after.resistances.effective.fire ?? 0).toBe(0);

    // The projection mutates a clone; the save the run saw is untouched.
    expect(source.weaponSet1[0]?.baseName).toBe(TWO_HANDER);
    expect(aggregateCharacter(source, gateDb, 'Ultimate').speed.attack.permanentPercent).toBe(26);
  });

  it('agrees with the same loadout aggregated directly, not just with itself', () => {
    const { input } = gateWorld();
    const projected = projectVerdicts(equipInMainHand('cand-1h'), input)!.after;

    // Built independently of the projection: a save that simply holds the
    // one-hander. If the gate lived only in one of the two paths, these differ.
    const worn = aggregateCharacter(
      save({
        weaponSet1: [instance({ baseName: ONE_HANDER, seed: 21 }), null],
        devotions: [characterSkill(STAR, 1)],
      }),
      gateDb,
      'Ultimate',
    );
    expect(projected.speed.attack.permanentPercent).toBe(worn.speed.attack.permanentPercent);
    expect(projected.resistances.effective.fire ?? 0).toBe(worn.resistances.effective.fire ?? 0);
    expect(projected.exclusions.join('\n')).toBe(worn.exclusions.join('\n'));
  });

  it('keeps the star when the projection equips a two-handed spear', () => {
    const { input } = gateWorld();
    const result = projectVerdicts(equipInMainHand('cand-spear'), input)!;

    // `WeaponMelee_Spear2h` satisfies `Spear2h`. Before the vocabulary fix the
    // restriction list had no spear entry at all, so this swap read as illegal
    // and the gate would have taken 26% attack speed off a spear user.
    expect(result.projection.skipped).toEqual([]);
    expect(result.after.speed.attack.permanentPercent).toBe(26);
    expect(result.after.resistances.effective.fire).toBe(30);
  });
});
