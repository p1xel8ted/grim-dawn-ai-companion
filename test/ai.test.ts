...
// ---------------------------------------------------------------------------
// What the prompt is allowed to say about attack speed
// ---------------------------------------------------------------------------

/**
 * The prompt used to tell the model that attack speed "multiplies every damage
 * figure in §4" and that at the cap it "is worth exactly nothing and giving it
 * up costs nothing". The first is wider than the tool's own arithmetic - the
 * throughput index is scoped to the main weapon attack precisely because a
 * cooldown, activated or auto-cast skill does not scale with attack rate. The
 * second is an instruction to act on a figure the tool knows disagrees with the
 * game. Both are gone; these cases keep them gone.
 */
describe('the advisor prompt on attack speed', () => {
  it('scopes the throughput claim to the weapon-attack channel', () => {
    expect(ADVISOR_SYSTEM_PROMPT).not.toContain('multiplies every damage figure');
    expect(ADVISOR_SYSTEM_PROMPT).toContain('the rate of the repeated main weapon attack');
    expect(ADVISOR_SYSTEM_PROMPT).toContain('the index does not model the other skills §4 lists');
    // The correction is about what the tool models, not a claim about the
    // engine: nothing here may say those skills are unaffected by speed.
    expect(ADVISOR_SYSTEM_PROMPT).toContain('does not say how they respond to attack speed either way');
  });

  it('refuses to license "at cap, so it is free to give up"', () => {
    expect(ADVISOR_SYSTEM_PROMPT).not.toContain('worth exactly nothing and giving it up costs nothing');
    expect(ADVISOR_SYSTEM_PROMPT).toContain('you may not conclude from them that more attack speed is worthless');
  });

  it('says the attack figures are an estimate, and that casting and movement are not', () => {
    expect(ADVISOR_SYSTEM_PROMPT).toContain("§3's attack figures are an estimate the tool knows disagrees with the game");
    expect(ADVISOR_SYSTEM_PROMPT).toContain('casting and movement figures carry no such doubt');
    // The document is still the source; the fix must not push the model back to
    // guessing speed off item lines.
    expect(ADVISOR_SYSTEM_PROMPT).toContain('never say the speed cannot be checked');
  });
});
