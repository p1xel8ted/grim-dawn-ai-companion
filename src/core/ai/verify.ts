...
function checkAvoidableHolds(
  ...
  for (const h of plan.hold) {
    ...
    warn(
      'avoidable-hold',
      `HOLD on ${name} for ${t.slot} waits on a drop, but on the projected totals the gap its swap opens (${gaps}) is closable — ` +
    ...
    if (claimed >= row.capAfter && shortfall > 2) {
      warn(
        ...
        `the tally claims ${row.label} Resistance at ${claimed} — at or over the ${row.capAfter} cap — but applying ` +
          `the plan's own verdicts computes ${row.after} on the projected totals, ${Math.round(shortfall)} short of cap; a listed ` +
